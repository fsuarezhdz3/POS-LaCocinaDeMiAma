...
export default function BebidasView({ onVolverMenu }) {
  ...
  // Lista de bebidas disponibles en catálogo (desde BD)
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [catBebidas, setCatBebidas] = useState([
    { id: 11, nombre: 'Jugo de Naranja Natural (500ml)', tipo: 'Bebida', precio: 35 },
    { id: 12, nombre: 'Café Americano de Olla', tipo: 'Bebida', precio: 25 },
    { id: 13, nombre: 'Refresco Embotellado (600ml)', tipo: 'Bebida', precio: 28 },
    { id: 14, nombre: 'Té Helado con Limón', tipo: 'Bebida', precio: 30 },
    { id: 15, nombre: 'Agua Embotellada Ciel (600ml)', tipo: 'Bebida', precio: 20 },
  ...
  // Preorden / Comanda activa de bebidas para esta mesa
  const [preorden, setPreorden] = useState([]);

  // Cargar catálogo de bebidas desde la BD
  useEffect(() => {
    cargarBebidasBD();
  }, []);

  const cargarBebidasBD = async () => {
    try {
      setLoading(true);
      ...
      if (res.ok && data.alimentos) {
        const bdBebidas = data.alimentos.filter(
          (a) => a.tipo === 'Bebida' || a.tipo === 'Litros'
        );
        if (bdBebidas.length > 0) {
          setCatBebidas(bdBebidas);
        }
      }
    } catch (err) {
      console.log('Utilizando catálogo local de respaldos para bebidas:', err.message);
    } finally {
      setLoading(false);
    }
  };

  // Agregar bebida a la preorden (o incrementar cantidad)
  const handleAgregarBebida = (item) => {
    setPreorden((prev) => {
  ...
  // Decrementar o eliminar bebida de la preorden
  const handleDecrementarBebida = (id) => {
    setPreorden((prev) => {
  ...
  // Eliminar bebida completamente de la preorden
  const handleEliminarBebida = (id) => {
    setPreorden((prev) => prev.filter((p) => p.id !== id));
  };

  // Actualizar comentario específico de una bebida
  const handleComentarioBebida = (id, texto) => {
    setPreorden((prev) =>
      prev.map((p) => (p.id === id ? { ...p, comentarios: texto } : p))
    );
  };

  // Calcular total acumulado de la preorden
  const calcularTotal = () => {
    return preorden.reduce((sum, p) => sum + Number(p.precio) * p.cantidad, 0);
  };

  // Enviar pedido a barra / cocina guardando en BD
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        showSuccessNotification(
          '¡Pedido Enviado!',
          `Pedido de Bebidas registrado exitosamente para ${selectedMesa ? selectedMesa.nombre : 'Mesa ' + numMesa}. Total: $${calcularTotal()}.00`
        );
      } else {
        Alert.alert('Error', data.error || 'No se pudo registrar la comanda');
      }
    } catch (err) {
      console.error('Error al enviar pedido de bebidas:', err);
      Alert.alert('Error de conexión', 'Verifica la conexión con el backend.');
    }
  };

  // Bebidas filtradas por búsqueda
  const bebidasFiltradas = catBebidas.filter((b) =>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Bebidas</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* SECCIÓN 1: PREORDEN DE BEBIDAS (ITEMS AGREGADOS) */}
        <View style={styles.preordenCard}>
          <View style={styles.preordenHeaderRow}>
            <Ionicons name="cart-outline" size={22} color="#0284C7" style={{ marginRight: 8 }} />
            <Text style={styles.preordenTitle}>Pre-orden de Bebidas</Text>
            {preorden.length > 0 && (
          ...
          {preorden.length === 0 ? (
            <View style={styles.emptyPreordenBox}>
              <Ionicons name="wine-outline" size={36} color="#CBD5E1" style={{ marginBottom: 6 }} />
              <Text style={styles.emptyPreordenText}>No has agregado bebidas a la comanda.</Text>
                  ...
                  <View style={styles.preordenItemBottomRow}>
                    <View style={styles.counterRow}>
                      <TouchableOpacity
                        ...
                        onPress={() => handleDecrementarBebida(item.id)}
                        activeOpacity={0.7}
                      >
                      ...
                      <TouchableOpacity
                        style={styles.btnCounter}
                        onPress={() => handleAgregarBebida(item)}
                        activeOpacity={0.7}
                      >
                    ...
                    <TextInput
                      ...
                      placeholder="Aclaraciones (ej. con hielo)..."
                      placeholderTextColor="#94A3B8"
                      value={item.comentarios}
                      onChangeText={(txt) => handleComentarioBebida(item.id, txt)}
                    />

                    <TouchableOpacity
                      style={styles.btnDeletePreordenItem}
                      onPress={() => handleEliminarBebida(item.id)}
                      activeOpacity={0.7}
                    >
        ...
        {/* SECCIÓN 2: CATÁLOGO DE BEBIDAS DESDE BD */}
        <View style={styles.catalogoHeaderRow}>
          <Text style={styles.catalogoSectionTitle}>Catálogo de Bebidas (BD)</Text>
        </View>
              ...
              <View key={item.id} style={styles.drinkCard}>
                <View style={styles.drinkCardLeft}>
                  <View style={styles.drinkIconBadge}>
                    <Ionicons name="wine-outline" size={20} color="#0284C7" />
                  </View>
                  <View style={styles.drinkInfoCol}>
                    <Text style={styles.drinkNombreText}>{item.nombre}</Text>
                    <Text style={styles.drinkTipoBadge}>{item.tipo}</Text>
                  </View>
                </View>
      ...
      <View style={styles.fixedBottomBar}>
        {preorden.length > 0 && (
          <View style={styles.totalSummaryRow}>
            <Text style={styles.totalSummaryLabel}>Total Bebidas:</Text>
