...
export default function DesayunosView({ onVolverMenu }) {
  ...
  // Lista de formularios de paquetes (Paquete 1, Paquete 2, ...)
  ...
  const cargarAlimentosBD = async () => {
    try {
      ...
      if (res.ok && data.alimentos) {
        ...
        if (bdDesayunos.length > 0) setCatDesayunos(bdDesayunos);
  ...
  // Agregar un nuevo formulario de paquete en blanco
  ...
  const handleSeleccionarOpcionBottomSheet = (opcion) => {
    ...
    setPaquetes((prev) =>
      prev.map((p) => {
        ...
        if (fieldType === 'desayuno') {
          const esMismo = p.desayuno?.id === opcion?.id;
          return { ...p, desayuno: esMismo ? null : opcion };
  ...
  const handleDeseleccionarCampo = () => {
    ...
    setPaquetes((prev) =>
      prev.map((p) => {
        ...
        if (fieldType === 'desayuno') return { ...p, desayuno: null };
  ...
  const obtenerPrecioBasePaquete = (p) => {
    // Verificar si se seleccionó absolutamente todo lo del paquete (desayuno, guarnición(es) y bebida)
    const tieneGuarnicion = p.guarnicion1 !== null || p.guarnicion2 !== null;
    const tieneBebida = p.bebida !== null;
    const paqueteCompleto = p.desayuno !== null && tieneGuarnicion && tieneBebida;

    // Helper para obtener el precio de un elemento según si aplica tarifa de paquete o precio normal
    ...
    const pDesayuno = getPrecioElemento(p.desayuno);
    ...
    return pDesayuno + pGuarnicion1 + pGuarnicion2 + pBebida;
  };

  const esPrecioPaqueteAplicado = (p) => {
    if (!p.desayuno) return false;
    const tieneGuarnicion = p.guarnicion1 !== null || p.guarnicion2 !== null;
    const tieneBebida = p.bebida !== null;
    return p.desayuno !== null && tieneGuarnicion && tieneBebida;
  };
    ...
    try {
      const itemsPayload = paquetes
        ...
        .map((p, idx) => {
          ...
          const alimentoConZona = p.desayuno
            ? `${p.desayuno.nombre}-${(p.desayuno.zona || 'cocina').toLowerCase()}`
            : null;
      ...
      if (res.ok && data.ok) {
        ...
        showSuccessNotification(
          '¡Pedido Enviado!',
          `Comanda de Desayunos registrada exitosamente para ${selectedMesa ? selectedMesa.nombre : 'Mesa ' + numMesa}. Total: $${calcularTotal()}.00`
        );
      } else {
        Alert.alert('Error', data.error || 'No se pudo registrar la comanda');
      }
    } catch (err) {
      console.error('Error al enviar comanda de desayuno:', err);
  ...
  return (
    ...
    >
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Desayunos</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {loading ? (
          <ActivityIndicator size="small" color="#D97706" style={{ marginVertical: 20 }} />
        ) : null}
        ...
        {paquetes.map((p) => {
          ...
          return (
            <View key={p.id} style={[styles.packageCard, { width: cardWidthResponsive }]}>
              {/* ENCABEZADO NEGRO CON BOTÓN X ROJO */}
              ...
              <View style={styles.packageBody}>
                ...
                <View style={styles.fieldRow}>
                  <Text style={styles.fieldLabel}>Desayuno:</Text>
                  <TouchableOpacity
                    style={[styles.fieldSlot, isActiveField('desayuno') && styles.fieldSlotActive]}
                    onPress={() => abrirBottomSheet(p.id, 'desayuno')}
                    activeOpacity={0.8}
                  >
                    <Text
                      style={[
                        styles.fieldSlotText,
                        p.desayuno && styles.fieldSlotTextSelected,
                      ]}
                      numberOfLines={1}
                    >
                      {p.desayuno ? `${p.desayuno.nombre} ($${getPrecioActivoElemento(p.desayuno, p)})` : 'Seleccionar Desayuno'}
                    </Text>
                    <Ionicons name="chevron-down" size={16} color="#64748B" />
                  </TouchableOpacity>
                </View>

                {/* 2. CAMPO GUARNICIONES (CON PRECIOS VISIBLES) */}
                <View style={styles.fieldRow}>
                ...
                {/* 3. CAMPO BEBIDA (CON PRECIO VISIBLE) */}
                <View style={styles.fieldRow}>
                ...
                <View style={styles.fieldRow}>
                  ...
                  <TextInput
                    ...
                    placeholder="Aclaraciones para cocina (ej. sin cebolla)..."
          ...
          <View style={styles.bottomSheetContainer}>
            ...
            <View style={styles.bottomSheetHeader}>
              <Text style={styles.bottomSheetTitle}>
                {activeTarget?.fieldType === 'desayuno' && 'Seleccionar Desayuno'}
            ...
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.bottomSheetList}>
              ...
              {/* OPCIONES DE DESAYUNO CON PRECIO */}
              {activeTarget?.fieldType === 'desayuno' &&
                catDesayunos.map((item) => {
                  const pActual = paquetes.find((p) => p.id === activeTarget?.paqueteId);
                  const isSelected = pActual?.desayuno?.id === item.id;
                  return (
                    <TouchableOpacity
                      key={item.id}
              ...
              {/* OPCIONES DE GUARNICIÓN CON PRECIO */}
