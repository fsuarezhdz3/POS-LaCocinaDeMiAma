...
export default function ExtrasView({ onVolverMenu }) {
  ...
  // Lista de extras disponibles en catálogo (desde BD)
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [catExtras, setCatExtras] = useState([
    { id: 31, nombre: 'Porción de Aguacate Hass', tipo: 'Extra', precio: 20 },
    { id: 32, nombre: 'Queso Gratinado Extra', tipo: 'Extra', precio: 15 },
    { id: 33, nombre: 'Porción de Tocino (3 tiras)', tipo: 'Extra', precio: 25 },
    { id: 34, nombre: 'Salsa Especial de la Casa', tipo: 'Extra', precio: 10 },
    { id: 35, nombre: 'Crema Fresca', tipo: 'Extra', precio: 10 },
    { id: 56, nombre: 'Orden de Tortillas Hechas a Mano (4 pz)', tipo: 'Extra', precio: 15 },
    { id: 57, nombre: 'Porción de Frijoles Refritos Extra', tipo: 'Extra', precio: 25 },
    { id: 58, nombre: 'Chiles Toreados con Cebollitas (2 pz)', tipo: 'Extra', precio: 20 },
  ]);

  // Preorden / Comanda activa de extras para esta mesa
  const [preorden, setPreorden] = useState([]);

  // Cargar catálogo de extras desde la BD
  useEffect(() => {
    cargarExtrasBD();
  }, []);

  const cargarExtrasBD = async () => {
    try {
      setLoading(true);
      ...
      if (res.ok && data.alimentos) {
        const bdExtras = data.alimentos.filter(
          (a) => a.tipo === 'Extra' || a.tipo === 'Extras'
        );
        if (bdExtras.length > 0) {
          setCatExtras(bdExtras);
        }
      }
    } catch (err) {
      console.log('Utilizando catálogo local de respaldos para extras:', err.message);
    } finally {
      setLoading(false);
    }
  };

  // Agregar extra a la preorden (o incrementar cantidad)
  const handleAgregarExtra = (item) => {
    setPreorden((prev) => {
  ...
  // Decrementar o eliminar extra de la preorden
  const handleDecrementarExtra = (id) => {
    setPreorden((prev) => {
  ...
  // Eliminar extra completamente de la preorden
  const handleEliminarExtra = (id) => {
    setPreorden((prev) => prev.filter((p) => p.id !== id));
  };

  // Actualizar comentario específico de un extra
  const handleComentarioExtra = (id, texto) => {
    setPreorden((prev) =>
      prev.map((p) => (p.id === id ? { ...p, comentarios: texto } : p))
    );
  };

  // Calcular total acumulado de la preorden de extras
  const calcularTotal = () => {
    return preorden.reduce((sum, p) => sum + Number(p.precio) * p.cantidad, 0);
  };

  // Enviar pedido de extras a cocina guardando en BD
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        showSuccessNotification(
          '¡Pedido Enviado!',
          `Pedido de Extras registrado exitosamente para ${selectedMesa ? selectedMesa.nombre : 'Mesa ' + numMesa}. Total: $${calcularTotal()}.00`
        );
      } else {
        Alert.alert('Error', data.error || 'No se pudo registrar la comanda');
      }
    } catch (err) {
      console.error('Error al enviar pedido de extras:', err);
      Alert.alert('Error de conexión', 'Verifica la conexión con el backend.');
    }
  };

  // Extras filtrados por búsqueda
  const extrasFiltrados = catExtras.filter((e) =>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Extras</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* SECCIÓN 1: PREORDEN DE EXTRAS (ITEMS AGREGADOS) */}
        <View style={styles.preordenCard}>
          <View style={styles.preordenHeaderRow}>
            <Ionicons name="add-circle-outline" size={22} color="#475569" style={{ marginRight: 8 }} />
            <Text style={styles.preordenTitle}>Pre-orden de Extras</Text>
            {preorden.length > 0 && (
          ...
          {preorden.length === 0 ? (
            <View style={styles.emptyPreordenBox}>
              <Ionicons name="add-circle-outline" size={36} color="#CBD5E1" style={{ marginBottom: 6 }} />
              <Text style={styles.emptyPreordenText}>No has agregado extras a la comanda.</Text>
                  ...
                  <View style={styles.preordenItemBottomRow}>
                    <View style={styles.counterRow}>
                      <TouchableOpacity
                        ...
                        onPress={() => handleDecrementarExtra(item.id)}
                        activeOpacity={0.7}
                      >
                    ...
                    <TextInput
                      ...
                      placeholder="Aclaraciones (ej. en contenedor aparte)..."
                      placeholderTextColor="#94A3B8"
                      value={item.comentarios}
                      onChangeText={(txt) => handleComentarioExtra(item.id, txt)}
                    />

                    <TouchableOpacity
                      style={styles.btnDeletePreordenItem}
                      onPress={() => handleEliminarExtra(item.id)}
                      activeOpacity={0.7}
                    >
        ...
        {/* SECCIÓN 2: CATÁLOGO DE EXTRAS DESDE BD */}
        <View style={styles.catalogoHeaderRow}>
          <Text style={styles.catalogoSectionTitle}>Catálogo de Extras (BD)</Text>
        </View>
        ...
        <View style={styles.searchContainer}>
          ...
          <TextInput
            ...
            placeholder="Buscar ingrediente extra por nombre..."
            placeholderTextColor="#94A3B8"
            value={searchQuery}
        ...
        {loading ? (
          <ActivityIndicator size="small" color="#475569" style={{ marginVertical: 20 }} />
        ) : (
          <View style={styles.drinksCatalogGrid}>
            {extrasFiltrados.map((item) => (
              <View key={item.id} style={styles.drinkCard}>
                <View style={styles.drinkCardLeft}>
                  ...
                  <View style={styles.drinkInfoCol}>
                    ...
                    <Text style={styles.drinkTipoBadge}>Extra</Text>
                  </View>
                </View>
      ...
      <View style={styles.fixedBottomBar}>
        {preorden.length > 0 && (
          <View style={styles.totalSummaryRow}>
            <Text style={styles.totalSummaryLabel}>Total Extras:</Text>
