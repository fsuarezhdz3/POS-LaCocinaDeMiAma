...
export default function LitrosView({ onVolverMenu }) {
  ...
  // Lista de litros disponibles en catálogo (desde BD)
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [catLitros, setCatLitros] = useState([
    { id: 36, nombre: 'Agua Fresca de Horchata (1 Litro)', tipo: 'Litros', precio: 50 },
    { id: 37, nombre: 'Agua Fresca de Jamaica (1 Litro)', tipo: 'Litros', precio: 50 },
    { id: 38, nombre: 'Agua Fresca de Limón con Chía (1 Litro)', tipo: 'Litros', precio: 50 },
    { id: 59, nombre: 'Caldo de Res Tradicional (1 Litro)', tipo: 'Litros', precio: 110 },
    { id: 60, nombre: 'Birria de Res con Caldo (1 Litro)', tipo: 'Litros', precio: 140 },
    { id: 61, nombre: 'Frijoles Charros Especiales (1 Litro)', tipo: 'Litros', precio: 75 },
    { id: 62, nombre: 'Consomé de Pollo (1 Litro)', tipo: 'Litros', precio: 80 },
  ]);

  // Preorden / Comanda activa de litros para esta mesa
  const [preorden, setPreorden] = useState([]);

  // Cargar catálogo de litros desde la BD
  useEffect(() => {
    cargarLitrosBD();
  }, []);

  const cargarLitrosBD = async () => {
    try {
      setLoading(true);
      ...
      if (res.ok && data.alimentos) {
        const bdLitros = data.alimentos.filter(
          (a) => a.tipo === 'Litros' || a.tipo === 'Litro'
        );
        if (bdLitros.length > 0) {
          setCatLitros(bdLitros);
        }
      }
    } catch (err) {
      console.log('Utilizando catálogo local de respaldos para litros:', err.message);
    } finally {
      setLoading(false);
    }
  };

  // Agregar litro a la preorden (o incrementar cantidad)
  const handleAgregarLitro = (item) => {
    setPreorden((prev) => {
  ...
  // Decrementar o eliminar litro de la preorden
  const handleDecrementarLitro = (id) => {
    setPreorden((prev) => {
  ...
  // Eliminar litro completamente de la preorden
  const handleEliminarLitro = (id) => {
    setPreorden((prev) => prev.filter((p) => p.id !== id));
  };

  // Actualizar comentario específico de un producto por litro
  const handleComentarioLitro = (id, texto) => {
    setPreorden((prev) =>
      prev.map((p) => (p.id === id ? { ...p, comentarios: texto } : p))
    );
  };

  // Calcular total acumulado de la preorden de litros
  const calcularTotal = () => {
    return preorden.reduce((sum, p) => sum + Number(p.precio) * p.cantidad, 0);
  };

  // Enviar pedido a cocina / barra guardando en BD
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        showSuccessNotification(
          '¡Pedido Enviado!',
          `Pedido por Litros registrado exitosamente para ${selectedMesa ? selectedMesa.nombre : 'Mesa ' + numMesa}. Total: $${calcularTotal()}.00`
        );
      } else {
        Alert.alert('Error', data.error || 'No se pudo registrar la comanda');
      }
    } catch (err) {
      console.error('Error al enviar pedido por litros:', err);
      Alert.alert('Error de conexión', 'Verifica la conexión con el backend.');
    }
  };

  // Litros filtrados por búsqueda
  const litrosFiltrados = catLitros.filter((l) =>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Litros</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* SECCIÓN 1: PREORDEN DE LITROS (ITEMS AGREGADOS) */}
        <View style={styles.preordenCard}>
          <View style={styles.preordenHeaderRow}>
            <Ionicons name="beaker-outline" size={22} color="#0D9488" style={{ marginRight: 8 }} />
            <Text style={styles.preordenTitle}>Pre-orden de Litros</Text>
            {preorden.length > 0 && (
          ...
          {preorden.length === 0 ? (
            <View style={styles.emptyPreordenBox}>
              <Ionicons name="beaker-outline" size={36} color="#CBD5E1" style={{ marginBottom: 6 }} />
              <Text style={styles.emptyPreordenText}>No has agregado litros a la comanda.</Text>
                  ...
                  <View style={styles.preordenItemBottomRow}>
                    <View style={styles.counterRow}>
                      <TouchableOpacity
                        ...
                        onPress={() => handleDecrementarLitro(item.id)}
                        activeOpacity={0.7}
                      >
                      ...
                      <TouchableOpacity
                        style={styles.btnCounter}
                        onPress={() => handleAgregarLitro(item)}
                        activeOpacity={0.7}
                      >
                    ...
                    <TextInput
                      ...
                      placeholder="Aclaraciones (ej. con poco hielo)..."
                      placeholderTextColor="#94A3B8"
                      value={item.comentarios}
                      onChangeText={(txt) => handleComentarioLitro(item.id, txt)}
                    />

                    <TouchableOpacity
                      style={styles.btnDeletePreordenItem}
                      onPress={() => handleEliminarLitro(item.id)}
                      activeOpacity={0.7}
                    >
        ...
        {/* SECCIÓN 2: CATÁLOGO DE LITROS DESDE BD */}
        <View style={styles.catalogoHeaderRow}>
          <Text style={styles.catalogoSectionTitle}>Catálogo de Litros (BD)</Text>
        </View>
      ...
      <View style={styles.fixedBottomBar}>
        {preorden.length > 0 && (
          <View style={styles.totalSummaryRow}>
            <Text style={styles.totalSummaryLabel}>Total Litros:</Text>
