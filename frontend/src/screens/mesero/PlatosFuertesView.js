...
export default function PlatosFuertesView({ onVolverMenu }) {
  ...
  // Lista de platos fuertes disponibles en catálogo a la carta (desde BD)
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [catPlatosFuertes, setCatPlatosFuertes] = useState([
    { id: 6, nombre: 'Milanesa de Res Empanizada A la Carta', tipo: 'Comida', precio: 110 },
    { id: 7, nombre: 'Pechuga Rellena de Queso y Jamón A la Carta', tipo: 'Comida', precio: 115 },
    { id: 8, nombre: 'Carne Asada con Ensalada A la Carta', tipo: 'Comida', precio: 120 },
    { id: 9, nombre: 'Enchiladas Verdes de Pollo A la Carta', tipo: 'Comida', precio: 95 },
    { id: 10, nombre: 'Flautas Doradas de Pollo (4 pz) A la Carta', tipo: 'Comida', precio: 85 },
    { id: 46, nombre: 'Mole Poblano con Pollo A la Carta', tipo: 'Comida', precio: 130 },
    { id: 47, nombre: 'Costillas de Puerco en Salsa Verde A la Carta', tipo: 'Comida', precio: 125 },
    { id: 48, nombre: 'Birria de Res estilo Jalisco A la Carta', tipo: 'Comida', precio: 135 },
  ]);

  // Preorden / Comanda activa de Platos Fuertes a la carta
  const [preorden, setPreorden] = useState([]);

  // Cargar catálogo de platos fuertes desde la BD
  useEffect(() => {
    cargarPlatosBD();
  }, []);

  const cargarPlatosBD = async () => {
    try {
      setLoading(true);
      ...
      if (res.ok && data.alimentos) {
        const bdPlatos = data.alimentos.filter(
          (a) => a.tipo === 'Comida' || a.tipo === 'Platos Fuertes'
        );
        if (bdPlatos.length > 0) {
          setCatPlatosFuertes(bdPlatos);
        }
      }
    } catch (err) {
      console.log('Utilizando catálogo local de respaldos para platos fuertes:', err.message);
    } finally {
      setLoading(false);
    }
  };

  // Agregar plato fuerte a la preorden (o incrementar cantidad)
  const handleAgregarPlato = (item) => {
    setPreorden((prev) => {
  ...
  // Decrementar o eliminar plato de la preorden
  const handleDecrementarPlato = (id) => {
    setPreorden((prev) => {
  ...
  // Eliminar plato completamente de la preorden
  const handleEliminarPlato = (id) => {
    setPreorden((prev) => prev.filter((p) => p.id !== id));
  };

  // Actualizar comentario específico de un plato fuerte
  const handleComentarioPlato = (id, texto) => {
    setPreorden((prev) =>
      prev.map((p) => (p.id === id ? { ...p, comentarios: texto } : p))
    );
  };

  // Calcular total acumulado de la preorden de platos fuertes
  const calcularTotal = () => {
    ...
    try {
      ...
      if (res.ok && data.ok) {
        ...
        showSuccessNotification(
          '¡Pedido Enviado!',
          `Pedido de Platos Fuertes registrado exitosamente para ${selectedMesa ? selectedMesa.nombre : 'Mesa ' + numMesa}. Total: $${calcularTotal()}.00`
        );
      } else {
        Alert.alert('Error', data.error || 'No se pudo registrar la comanda');
      }
    } catch (err) {
      console.error('Error al enviar pedido de platos fuertes:', err);
      Alert.alert('Error de conexión', 'Verifica la conexión con el backend.');
    }
  };

  // Platos filtrados por búsqueda
  const platosFiltrados = catPlatosFuertes.filter((p) =>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Platos Fuertes</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* SECCIÓN 1: PREORDEN DE PLATOS FUERTES (ITEMS AGREGADOS) */}
        <View style={styles.preordenCard}>
          <View style={styles.preordenHeaderRow}>
            <Ionicons name="flame-outline" size={22} color="#E11D48" style={{ marginRight: 8 }} />
            <Text style={styles.preordenTitle}>Pre-orden A la Carta</Text>
            {preorden.length > 0 && (
          ...
          {preorden.length === 0 ? (
            <View style={styles.emptyPreordenBox}>
              <Ionicons name="flame-outline" size={36} color="#CBD5E1" style={{ marginBottom: 6 }} />
              <Text style={styles.emptyPreordenText}>No has agregado platillos a la comanda.</Text>
              <Text style={styles.emptyPreordenSub}>Toca '+ Agregar' en la lista de abajo para añadir a la carta.</Text>
            </View>
          ) : (
                  ...
                  <View style={styles.preordenItemBottomRow}>
                    <View style={styles.counterRow}>
                      <TouchableOpacity
                        ...
                        onPress={() => handleDecrementarPlato(item.id)}
                        activeOpacity={0.7}
                      >
                    ...
                    <TextInput
                      ...
                      onChangeText={(txt) => handleComentarioPlato(item.id, txt)}
                    />
        ...
        {/* SECCIÓN 2: CATÁLOGO DE PLATOS FUERTES DESDE BD */}
        <View style={styles.catalogoHeaderRow}>
          <Text style={styles.catalogoSectionTitle}>Platos Fuertes A la Carta (BD)</Text>
        </View>
              ...
              <View key={item.id} style={styles.drinkCard}>
                <View style={styles.drinkCardLeft}>
                  <View style={styles.drinkIconBadge}>
                    <Ionicons name="flame-outline" size={20} color="#E11D48" />
                  </View>
                  <View style={styles.drinkInfoCol}>
                    <Text style={styles.drinkNombreText}>{item.nombre}</Text>
                    <Text style={styles.drinkTipoBadge}>Plato Fuerte</Text>
                  </View>
                </View>
      ...
      <View style={styles.fixedBottomBar}>
        {preorden.length > 0 && (
          <View style={styles.totalSummaryRow}>
            <Text style={styles.totalSummaryLabel}>Total Platos Fuertes:</Text>
