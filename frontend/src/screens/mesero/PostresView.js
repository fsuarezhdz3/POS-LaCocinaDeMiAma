...
export default function PostresView({ onVolverMenu }) {
  ...
  // Lista de postres disponibles en catálogo (desde BD)
  const [loading, setLoading] = useState(false);
  const [searchQuery, setSearchQuery] = useState('');
  const [catPostres, setCatPostres] = useState([
    { id: 16, nombre: 'Flan Napolitano Casero', tipo: 'Postre', precio: 45 },
    { id: 17, nombre: 'Pastel de Tres Leches con Durazno', tipo: 'Postre', precio: 55 },
    { id: 18, nombre: 'Pay de Limón helado', tipo: 'Postre', precio: 40 },
    { id: 19, nombre: 'Arroz con Leche y Canela', tipo: 'Postre', precio: 35 },
    { id: 20, nombre: 'Helado de Vainilla o Chocolate (2 bolas)', tipo: 'Postre', precio: 30 },
    { id: 39, nombre: 'Carlota de Limón Tradicional', tipo: 'Postre', precio: 40 },
    { id: 40, nombre: 'Churros Azucarados con Cajeta (4 pz)', tipo: 'Postre', precio: 45 },
  ]);

  // Preorden / Comanda activa de postres para esta mesa
  const [preorden, setPreorden] = useState([]);

  // Cargar catálogo de postres desde la BD
  useEffect(() => {
    cargarPostresBD();
  }, []);

  const cargarPostresBD = async () => {
    try {
      setLoading(true);
      ...
      if (res.ok && data.alimentos) {
        const bdPostres = data.alimentos.filter(
          (a) => a.tipo === 'Postres' || a.tipo === 'Postre'
        );
        if (bdPostres.length > 0) {
          setCatPostres(bdPostres);
        }
      }
    } catch (err) {
      console.log('Utilizando catálogo local de respaldos para postres:', err.message);
    } finally {
      setLoading(false);
    }
  };

  // Agregar postre a la preorden (o incrementar cantidad)
  const handleAgregarPostre = (item) => {
    setPreorden((prev) => {
  ...
  // Decrementar o eliminar postre de la preorden
  const handleDecrementarPostre = (id) => {
    setPreorden((prev) => {
  ...
  // Eliminar postre completamente de la preorden
  const handleEliminarPostre = (id) => {
    setPreorden((prev) => prev.filter((p) => p.id !== id));
  };

  // Actualizar comentario específico de un postre
  const handleComentarioPostre = (id, texto) => {
    setPreorden((prev) =>
      prev.map((p) => (p.id === id ? { ...p, comentarios: texto } : p))
    );
  };

  // Calcular total acumulado de la preorden de postres
  const calcularTotal = () => {
    return preorden.reduce((sum, p) => sum + Number(p.precio) * p.cantidad, 0);
  };

  // Enviar pedido de postres a cocina / repostería guardando en BD
  const handleEnviarPedido = async () => {
    ...
    try {
      const itemsPayload = preorden.map((p) => ({
        alimento: `${p.cantidad}x ${p.nombre}-barra`,
      ...
      if (res.ok && data.ok) {
        ...
        showSuccessNotification(
          '¡Pedido Enviado!',
          `Pedido de Postres registrado exitosamente para ${selectedMesa ? selectedMesa.nombre : 'Mesa ' + numMesa}. Total: $${calcularTotal()}.00`
        );
      } else {
        Alert.alert('Error', data.error || 'No se pudo registrar la comanda');
      }
    } catch (err) {
      console.error('Error al enviar pedido de postres:', err);
      Alert.alert('Error de conexión', 'Verifica la conexión con el backend.');
    }
  };

  // Postres filtrados por búsqueda
  const postresFiltrados = catPostres.filter((p) =>
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.topBar}>
        ...
        <Text style={styles.topTitle}>Postres</Text>
      </View>
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        {/* SECCIÓN 1: PREORDEN DE POSTRES (ITEMS AGREGADOS) */}
        <View style={styles.preordenCard}>
          <View style={styles.preordenHeaderRow}>
            <Ionicons name="ice-cream-outline" size={22} color="#EC4899" style={{ marginRight: 8 }} />
            <Text style={styles.preordenTitle}>Pre-orden de Postres</Text>
            {preorden.length > 0 && (
          ...
          {preorden.length === 0 ? (
            <View style={styles.emptyPreordenBox}>
              <Ionicons name="ice-cream-outline" size={36} color="#CBD5E1" style={{ marginBottom: 6 }} />
              <Text style={styles.emptyPreordenText}>No has agregado postres a la comanda.</Text>
                  ...
                  <View style={styles.preordenItemBottomRow}>
                    <View style={styles.counterRow}>
                      <TouchableOpacity
                        ...
                        onPress={() => handleDecrementarPostre(item.id)}
                        activeOpacity={0.7}
                      >
                      ...
                      <TouchableOpacity
                        style={styles.btnCounter}
                        onPress={() => handleAgregarPostre(item)}
                        activeOpacity={0.7}
                      >
                    ...
                    <TextInput
                      ...
                      placeholder="Aclaraciones (ej. con cajeta extra)..."
                      placeholderTextColor="#94A3B8"
                      value={item.comentarios}
                      onChangeText={(txt) => handleComentarioPostre(item.id, txt)}
                    />

                    <TouchableOpacity
                      style={styles.btnDeletePreordenItem}
                      onPress={() => handleEliminarPostre(item.id)}
                      activeOpacity={0.7}
                    >
        ...
        {/* SECCIÓN 2: CATÁLOGO DE POSTRES DESDE BD */}
        <View style={styles.catalogoHeaderRow}>
          <Text style={styles.catalogoSectionTitle}>Catálogo de Postres (BD)</Text>
        </View>
      ...
      <View style={styles.fixedBottomBar}>
        {preorden.length > 0 && (
          <View style={styles.totalSummaryRow}>
            <Text style={styles.totalSummaryLabel}>Total Postres:</Text>
